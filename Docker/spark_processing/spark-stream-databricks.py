...
# write to POSTGRES
# ==========================================
def write_to_databricks(batch_df, batch_id):
    batch_df.write \
        .format("jdbc") \
        .option("url", "jdbc:databricks://<host>:443/default") \
        .option("dbtable", "trades") \
        .option("user", "token") \
        .option("password", "<access-token>") \
        .mode("append") \
        .save()


databricks_query = clean_df.writeStream \
    .foreachBatch(write_to_databricks) \
    .outputMode("append") \
    .start()
